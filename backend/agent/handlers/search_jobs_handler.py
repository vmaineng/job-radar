...
DEFAULT_SEARCH_TITLES = [
    t.strip() for t in os.getenv("SEARCH_TITLES", "").split(",") if t.strip()
]
...
async def search_jobs_handler(
    ...
    raw_jobs = await run_search_profile(titles, location, remote)
    new_jobs = [
        j
        for j in raw_jobs
        if not job_already_seen(user_id, j["source"], j["external_id"])
    ]
